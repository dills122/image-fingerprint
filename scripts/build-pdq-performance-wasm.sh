...
usage() {
  printf '%s\n' \
    ...
    "" \
    "Builds a disposable in-process PDQ performance comparator with pinned Emscripten."
}
...
command -v docker >/dev/null 2>&1 || {
  printf 'Docker is required to build the WASM comparator\n' >&2
  exit 2
}
...
case "$OUTPUT_DIRECTORY/" in
  "$REPOSITORY_ROOT/"*)
    printf 'Comparator output must be outside the repository: %s\n' "$OUTPUT_DIRECTORY" >&2
    exit 2
    ;;
esac
...
docker run --rm --platform linux/amd64 \
  ...
  "$EMSCRIPTEN_IMAGE" \
  em++ \
  /work/tools/pdq-performance-wasm/main.cpp \
  ...
  -s EXPORTED_FUNCTIONS='["_pdq_hash_rgb","_malloc","_free"]' \
  --no-entry \
  -o /out/pdq-performance.wasm

printf '%s\n' \
  "pdq_performance_wasm=$OUTPUT_DIRECTORY/pdq-performance.wasm" \
  "reference_commit=$ACTUAL_COMMIT" \
  "emscripten_image=$EMSCRIPTEN_IMAGE" \
  "flags=-std=c++11 -O3 -ffp-contract=off -Wall -Wextra -Werror -s STANDALONE_WASM=1 -s ALLOW_MEMORY_GROWTH=1"
